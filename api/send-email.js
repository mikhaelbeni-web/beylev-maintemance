...
module.exports = async (req, res) => {
  ...
  try{
    ...
    const html=`<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto"><div style="background:#1e1b4b;color:white;padding:20px;border-radius:12px 12px 0 0"><h2 style="margin:0">🔧 ${p.subject}</h2></div><div style="background:#f8fafc;padding:20px;border-radius:0 0 12px 12px;border:1px solid #e2e8f0"><p>Bonjour <strong>${p.to_name}</strong>,</p><p>${p.message}</p><div style="background:white;border-left:4px solid #f59e0b;border-radius:8px;padding:16px;margin:16px 0"><p style="margin:4px 0"><strong>🔧</strong> ${p.mission_title}</p><p style="margin:4px 0"><strong>📅</strong> ${p.mission_date}</p><p style="margin:4px 0"><strong>📍</strong> ${p.mission_location}</p><p style="margin:4px 0"><strong>👤</strong> Assigné à ${p.assigned_to}</p><p style="margin:4px 0"><strong>⚡</strong> ${p.mission_priority}</p></div><a href="${p.app_url}" style="display:inline-block;background:#6366f1;color:white;padding:10px 20px;border-radius:8px;text-decoration:none;font-weight:bold">→ Voir l\'application</a><p style="color:#94a3b8;font-size:12px;margin-top:20px">Maintenance Beylev</p></div></div>`;
